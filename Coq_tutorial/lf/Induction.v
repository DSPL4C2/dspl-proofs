...
Proof.
  ...
  - intros m. simpl. rewrite -> IH. rewrite -> plus_n_Sm.
    reflexivity.
Qed.
...
Proof.
  induction n as [| n' IH].
  - reflexivity.
  - simpl. rewrite <- plus_n_Sm. rewrite -> IH.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  induction n as [| n' IH].
  { reflexivity. }
  { rewrite -> IH. rewrite -> negb_involutive.
    reflexivity. }
Qed.
(** [] *)
...
(* Enquanto a tática [destruct] divide uma variável em seus possíveis
   casos, e então o problema é resolvido para cada caso, na tática [induction]
   temos que fazer uma indução estrutural na forma como tal variável é construida,
   provando assim a base de indução e o passo de indução a partir da 
   hipótese de indução*)
*)
...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    Proof: Para provar a comutatividade da soma de m e n iremos usar indução
    em n.

    Base de indução: temos que 0 + m = m + 0, o que pela definição de + e 
    sabendo que m + 0 = m para todo m leva a prova.

    Passo de indução: dado que n' + m = m + n' provamos que S n' + m = 
    m + S n'. Pela definição de S e + temos que S(n' + m) = m + S n'. Pela
    hipítese de indução e sabendo que m + S n' = S (m + n') finalizamos a prova.
    Qed.
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)  
    ...
    Proof: Vamos provar isso por indução em n.

    Base de indução: Para n = 0 temos que a afirmação é trivial.

    Passo de indução: Para um n na forma S n', temos como hipótese que n' == n',
    e para o caso S n' =? S n' temos que provar que n' == n' pela definição
    de =?, o que é nossa hipótese, finalizando assim a prova. Qed. 
...
Proof.
  ...
  assert (m + p = p + m) as H'.
  { rewrite -> plus_comm. reflexivity. }
  rewrite -> H'. rewrite -> H. rewrite -> plus_assoc. reflexivity.
Qed.
...
Proof.
  intros m n p. intros H. induction p as [| p' IH].
...
Proof.
  intros n. simpl. induction n as [| n' IH].
  - reflexivity.
  - simpl. rewrite -> IH. reflexivity.
Qed.
...
Proof.
  induction b as[|z' IH'| z'' IH'].
  - reflexivity.
  - simpl. destruct z'.
    + reflexivity.
    + assert (H:forall m n, m + n + 1 = S(m + n)).
      { intros m n. rewrite <- plus_comm. reflexivity. }
      rewrite -> H. reflexivity.
    + assert (H:forall m n, m + n + 1 = S(m + n)).
      { intros m n. rewrite <- plus_comm. reflexivity. }
      rewrite -> H. reflexivity.
  - simpl. destruct z''.
    + reflexivity.
    + rewrite -> IH'. 
      assert (H: S(bin_to_nat(A z'')) + 0 = (bin_to_nat(A z'') + 0) + 1).
      { rewrite <- plus_1_l. rewrite <- plus_n_O. rewrite <- plus_n_O.
        rewrite -> plus_comm. reflexivity. }
      rewrite -> H. rewrite -> plus_assoc. reflexivity.
    + rewrite -> IH'. 
      assert (H: S(bin_to_nat(B z'')) + 0 = (bin_to_nat(B z'') + 0) + 1).
      { rewrite <- plus_1_l. rewrite <- plus_n_O. rewrite <- plus_n_O.
        rewrite -> plus_comm. reflexivity. }
      rewrite -> H. rewrite -> plus_assoc. reflexivity.
Qed.
...
Theorem aux : forall a: nat, a + a + (a + a + 0) + 1 = S(a + a + (a + a + 0)).
Proof.
  intros a.
  assert (H:forall x y, x + y + 1 = S(x + y)).
...
Theorem aux' : forall a: nat, a + a + 1 + (a + a + 1) = S (a + a + (a + a + 0) + 1).
Proof.
  intros a. assert(H: forall x: nat, x + 1 + (x + 1) = S(x + (x + 0) + 1)).
  - intros x. rewrite <- plus_n_O. rewrite -> plus_assoc. 
    rewrite -> plus_comm. simpl. rewrite -> plus_comm. rewrite ->plus_assoc.
    reflexivity.
  - rewrite -> H. reflexivity.
Qed.

Theorem aux'': forall a,
  a + a + 1 + (a + a + 1 + 0) + (a + a + 1 + (a + a + 1 + 0) + 0) = 
  S (a + a + (a + a + 0) + 1 + (a + a + (a + a + 0) + 1 + 0) + 1).
Proof.
  intros a. rewrite <- plus_n_O. 
  assert (H: forall x, x+1+(x+1)+(x+1+(x+1)+0) = S(x+(x+0)+1+(x+(x+0)+1+0)+1)).
  -intros x. rewrite <- plus_n_O. rewrite <- plus_n_O. 
   rewrite -> plus_assoc. rewrite -> plus_swap. rewrite -> plus_comm.
Abort.
...
Proof.
  induction n as [| n' IH].
  - reflexivity.
  - simpl. destruct (nat_to_bin n') as [| n''| n''] eqn: H'.
    + rewrite <- IH. reflexivity.
    + simpl. destruct n'' eqn: H''.
      { rewrite <- IH. reflexivity. }
      { rewrite <- IH. simpl. rewrite <- plus_n_O. rewrite -> aux. reflexivity. }
      { rewrite <- IH. rewrite <- H''. simpl. rewrite -> plus_comm. simpl.
        reflexivity. }
    + simpl. destruct n'' eqn: H''.
      { rewrite <- IH. reflexivity. }
      { rewrite <- IH. simpl. destruct b.
        { reflexivity. }
        { rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite -> aux'. reflexivity. }
        { rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite -> aux'. reflexivity. }}
      { rewrite <- IH. destruct b.
        { reflexivity. }
        { simpl. destruct b.
          { reflexivity. }
          { rewrite <- plus_n_O.
