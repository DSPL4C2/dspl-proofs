...
Definition ltb (n m : nat) : bool := (leb n m) && (negb (leb m n)).
...
Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof.
  intros n m o. intros H1 H2. rewrite -> H1. rewrite <- H2. reflexivity. Qed.
(** [] *)
...
Example test_bin_incr1:  bin_to_nat(incr(B(B(A(B(A(B Z))))))) = 44.
Proof. reflexivity. Qed.

Example test_bin_incr2:  bin_to_nat(incr(A(A(A(B(B(A(B Z)))))))) = 89.
Proof. reflexivity. Qed.

Example test_bin_incr3:  bin_to_nat(incr(B(B(B(B(B(B Z))))))) = 64.
Proof. reflexivity. Qed.

Example test_bin_incr4:  bin_to_nat(incr(B(B(B(B(B(B(B Z)))))))) = 128.
Proof. reflexivity. Qed.

Example test_bin_incr5:  bin_to_nat(incr(A(A(A(B(B(B(A(B Z))))))))) = 185.
Proof. reflexivity. Qed.
